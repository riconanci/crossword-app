...
import { puzzleFingerprint } from "../lib/puzzleHistory";
...
export default class CrosswordServer implements Party.Server {
  ...
  async onStart() {
    ...
    const savedHistory = await this.room.storage.get<string[]>("playedFingerprints");
    if (saved) this.roomState = saved;
    if (savedPuzzle) this.puzzleWithAnswers = savedPuzzle;
    if (savedHistory) this.playedFingerprints = savedHistory;
  }
  ...
  private async handleStartGame(mode: GameMode, size: BoardSize, sender: Party.Connection) {
    ...
    // Generate puzzle — retry up to 5 times to avoid a repeat
    let puzzleWithAnswers = generatePuzzle(size);
    for (let i = 0; i < 5; i++) {
      const fp = puzzleFingerprint(puzzleWithAnswers.answers);
      if (!this.playedFingerprints.includes(fp)) break;
      puzzleWithAnswers = generatePuzzle(size);
    }
    // Record this puzzle in history
    const fp = puzzleFingerprint(puzzleWithAnswers.answers);
    this.playedFingerprints = [fp, ...this.playedFingerprints].slice(0, 20);
  ...
  private async persist() {
    ...
    await this.room.storage.put("playedFingerprints", this.playedFingerprints);
  }
}
