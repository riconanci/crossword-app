...
import { soundKeypress, soundDelete, soundWordComplete } from "@/lib/sounds";
import { hapticKeypress, hapticDelete, hapticWordComplete } from "@/lib/haptics";
...
export function useGridInput(
  ...
  /** Cells locked as correct — cannot be overwritten or deleted */
  correctCells: number[] = []
): UseGridInputReturn {
  ...
  const handleKey = useCallback(
    (key: string) => {
      ...
      if (/^[a-zA-Z]$/.test(key)) {
        ...
        // Don't overwrite locked (verified correct) cells
        if (!correctCells.includes(selection.cellIndex)) {
          soundKeypress();
          hapticKeypress();
        ...
        if (entries[selection.cellIndex] && !correctCells.includes(selection.cellIndex)) {
          ...
          if (clue) {
            ...
            for (let i = pos - 1; i >= 0; i--) {
              const ci = clue.cells[i]!;
              if (!correctCells.includes(ci)) { prev = ci; break; }
            }
            if (prev !== null) {
              onInput(prev, "");
      ...
      // ── Delete ────────────────────────────────────────────────────────────
      if (key === "Delete") { if (!correctCells.includes(selection.cellIndex)) { soundDelete(); hapticDelete(); onInput(selection.cellIndex, ""); } return; }
  ...
  // ── Auto-select 1-Across when a puzzle loads ──────────────────────────────
  const prevPuzzleId = useRef<string | null>(null);
  useEffect(() => {
    if (!puzzle) return;
    if (puzzle.id === prevPuzzleId.current) return; // same puzzle, don't reset
    prevPuzzleId.current = puzzle.id;

    // Find the first across clue (already sorted: across first, then down)
    const firstAcross = puzzle.clues.find((c) => c.direction === "across");
    if (!firstAcross) return;
